...
namespace RedisTr.Controllers
{
    ...
    public class ApiCore : ControllerBase
    {
        ...
        public async Task<MDataOut> PostCore([FromBody] MDataIn mDataIn)
        {
            ...
            if (string.IsNullOrEmpty(rawBody)) throw new Exception("Не могу получить тело запроса");
            var sub = _connectionFactory.GConnectionMultiplexer().GetSubscriber();
            string resp = string.Empty;
            try
            {
                
                resp = await sub.CallerRcpExtAsync(Utils.ChanelName(mDataIn), rawBody);
            ...
            {
                mDataOut.SetErrorData(ErrorCode.PrintError, e.Message);
